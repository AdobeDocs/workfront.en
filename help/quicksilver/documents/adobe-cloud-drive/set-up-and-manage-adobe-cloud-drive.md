...
* **Workfront project folders in Finder and File Explorer:** Users see the Workfront projects they have access to, organized as folders in the mounted drive.
* **Works with any desktop application:** Files open from the desktop in Photoshop, Premiere Pro, Microsoft Word, or any other installed application.
* **Offline access:** Users select **Always Keep On This Device** to pin files for offline use, or **Free Up Space** to remove local copies while keeping files available in the cloud.
* **Conflict protection:** When two users save conflicting edits to the same file, Adobe Cloud Drive preserves both versions automatically. It doesn't use file locking.
* **Read-only project root folders:** Users can't rename, move, or delete a project root folder from the desktop. They can create folders, subfolders, and files at any depth inside a project folder.

### Limitations

* **No auto-sync.** Users must save explicitly using **File** > **Save**. Changes aren't pushed to the cloud automatically when a file is closed or auto-saved.
* **No new project folder creation from the desktop.** Workfront project folders are created in Workfront. Users can't add a new top-level project folder from Finder or File Explorer.
* **Workfront project assets only.** Access to other Adobe cloud storage locations isn't currently available.

## How access is controlled in the Adobe Admin Console

Adobe Cloud Drive is included with Workfront Ultimate. It doesn't appear as a standalone product in the **Products** section of the Adobe Admin Console. Instead, it's managed through the **Roles** section under **Users**.
...
| --- | --- | --- |
| **Member** | All users in the organization | Contains the org-level Adobe Cloud Drive capability toggle. On by default. |
| **ACD user** | No one, by default | Grants individual access when the org-level toggle is off. |

### The two access controls

**Control 1: The org-level capability toggle (in the Member role)**

The **Member** role is automatically assigned to every user in your organization. Within this role, there's an Adobe Cloud Drive capability toggle. When this toggle is on, every user with a Workfront Ultimate license can access Adobe Cloud Drive. When it's off, no user can access Adobe Cloud Drive, regardless of their license.

The toggle is on by default when Adobe activates Adobe Cloud Drive for your organization.

**Control 2: The ACD user role**

The **ACD user** role is only relevant when the org-level toggle is off. If you turn off the org-level toggle to run a controlled pilot, you can still grant access to specific users by adding them to the **ACD user** role. Users in this role can access Adobe Cloud Drive even when the org-level toggle is off. If the org-level toggle is on, the **ACD user** role has no effect.

**Underlying requirement: Workfront Ultimate license**

Both controls apply only to users who have a Workfront Ultimate license. A user without a Workfront Ultimate license can't access Adobe Cloud Drive, regardless of how the toggle or role is configured.

The following table shows how these controls interact:

| Org-level toggle | User in ACD user role | Workfront Ultimate license | Access result |
| --- | --- | --- | --- |
...
* The users you plan to provision have Workfront Ultimate licenses assigned.
...
>[!NOTE]
>
>A user who has access enabled but doesn't have access to any Workfront projects sees an empty mounted drive after sign-in. This is expected. Workfront project membership is managed separately in Workfront.

## Step 1: Configure access in the Adobe Admin Console
...
When Adobe activates Adobe Cloud Drive for your organization, the org-level capability toggle is on by default, and all Workfront Ultimate users have access immediately. Use this procedure to confirm the toggle is on before you deploy the application.
...
1. In the **Member** panel that opens, confirm that **Adobe Cloud Drive** appears under **Permissions** and its toggle is on.
...
Use this option when you want to limit access to a defined set of users — for example, during a pilot before a broader rollout. This involves turning off the org-level toggle, then adding your pilot users to the **ACD user** role.

>[!IMPORTANT]
>
>Completing the first procedure below removes Adobe Cloud Drive access for all users in your organization immediately, including users who are currently signed in. Complete both procedures in the same session, without delay.

#### Turn off the org-level capability

1. Sign in to [adminconsole.adobe.com](https://adminconsole.adobe.com/).
1. Click **Users** in the top navigation bar, then click **Roles** in the left panel.
1. Click **Member** in the roles list.
1. In the **Member** panel, locate **Adobe Cloud Drive** under **Permissions** and toggle it off.
1. Click **Save**.

#### Add pilot users to the ACD user role
...
>[!TIP]
>
>To expand access over time, return to the **ACD user** role and add users as needed. When you're ready for a full rollout, turn the org-level toggle back on in the **Member** role. Once the org-level toggle is on, the **ACD user** role has no effect and doesn't need to be maintained.

## Step 2: Deploy the Adobe Cloud Drive application
...
Adobe Cloud Drive is a standalone application. It isn't distributed through the Creative Cloud desktop application and doesn't appear in the Creative Cloud package manager.
...
#### Create the package in the Adobe Admin Console
...
1. In the **Finalize** step, enter a name for the package, select **Flat package**, review the summary, and click **Create package**.
...
Use this method for smaller organizations, self-managed devices, or when directing individual users to install the application themselves.
...
1. Direct the user to [experience.adobe.com/#/downloads](https://experience.adobe.com/#/downloads).
   ...
   >[!NOTE]
   >
   >Users must have Adobe Cloud Drive access enabled in the Adobe Admin Console to see the Adobe Cloud Drive installer. Users without access don't see the installer listed.
...
1. Confirm that Workfront project folders appear in the mounted drive in Finder or File Explorer.
   ...
   >[!NOTE]
   >
   >A user who signs in successfully but sees no folders has no Workfront project membership. Add the user to a project in Workfront to populate the drive.

1. Navigate into a project folder and create a small test file.
1. Open Workfront in a browser and confirm the test file appears in the corresponding project.
