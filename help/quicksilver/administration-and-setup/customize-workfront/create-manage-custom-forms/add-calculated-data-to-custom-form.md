...
1. (Optional) Use any of the following options to configure your calculated custom field:
   ...
   <table cellspacing="0"> 
    ...
    <tbody> 
     ...
     </tr> Display formula in instructions Leave this option enabled if you want the users who fill out the custom form to see the field's formula when they hover over the field. For more information, see the information about Instructions earlier in this table. 
    </tbody> 
   </table>
...
<!--
Calculated custom fields in multi-object custom forms In a multi-object custom form, the calculations in all calculated custom fields must be compatible with all of the form's object types. If you add one that is incompatible with the other, a warning message alerts you so that you can make adjustments. Example: You create a form and associate it with the Task and Issue object types. An calculation in one of the form's calculated custom fields contains the Percent Complete field. Later, you add the User object type to the custom form. A warning message tells you that the User object type is incompatible with the Percent Complete field. When this occurs, you can do one of the following: Remove one of the two incompatibilities—either the object type or the expression. Use the wildcard $$OBJcode as a condition in an IF expression to create different versions of the custom field that work with the different object types. Example: You create a form and associate it with the Project and Task object types. You want to add a calculated custom field called Owner that indicates who is in charge of the work when the form is attached to a project or task. But, in a project, the owner is the person who created the project; in a task, it's the primary person assigned to work on the task. To accommodate this difference in your Owner custom field, you could create the following expression: IF($$OBJCODE=“PROJ”,{owner:name},{assignedToID:name}) Then, when someone attaches the form to a project, the Owner field displays the name of the Project Owner. When someone attaches it to a task, the same field displays the name of the primary person assigned to the task.
-->
