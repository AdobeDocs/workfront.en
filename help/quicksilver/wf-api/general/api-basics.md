...
>[!NOTE]
>
>The examples in this article include `<supported-version>`. Replace this with the version of the Workfront API that you want to use.
>For the Workfront API versioning and support schedule, see [API versioning and support schedule](/help/quicksilver/wf-api/api/api-version-support-schedule.md). 
...
1. Change the URL to /attask/api/&lt;supported-version&gt;/project/search.  
...
1. Change the URL back to **/attask/api/&lt;supported-version&gt;/project/search**.
...
Notice the /attask/api/&lt;supported-version&gt;/project?id=... request is the same as the `/attask/api/&lt;supported-version&gt;/project/...` request.
...
<pre>/attask/api/&lt;supported-version&gt;/user/search?username=testuser@workfront.com</pre>pass the URL parameter in a filter, as shown in the following example:
...
<pre>/attask/api/&lt;supported-version&gt;/task/search?map=true</pre>returns a response indexed by ID similar to the following:
...
<pre>/attask/api/&lt;supported-version&gt;/task/search?fields=plannedStartDate,priority</pre>returns a response similar to the following:
...
<pre>/attask/api/&lt;supported-version&gt;/issue/search?fields=owner</pre>If more information is required, you can request a nested field using colon syntax. For example, the following request searches for all issues along with the owner's name, ID, title, and phone number
<pre>/attask/api/&lt;supported-version&gt;/issue/search?fields=owner:title,owner:phoneNumber</pre>and returns the following:&nbsp;
...
<pre>/attask/api/&lt;supported-version&gt;/project/search?fields=tasks</pre>The following request gets task assignments:
...
<pre>/attask/api/&lt;supported-version&gt;/project/search?fields=DE:CustomText</pre>which would return
...
<pre>/attask/api/&lt;supported-version&gt;/project/search?fields=parameterValues</pre>returns similar data to the following:
...
<pre>GET /attask/api/&lt;supported-version&gt;/project/count?status=CUR</pre>returns the number of results in the following format:
...
<pre>GET /attask/api/&lt;supported-version&gt;/hour/report?project:name_1_GroupBy=true&hours_AggFunc=sum</pre>which returns the following result
...
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx?method=put &updates={ accessRules: [ {accessorID: 'abc123', accessorObjCode: 'USER', coreAction: 'VIEW'} ] }</pre>Alternatively, to share only with a new person and keep existing permissions intact:
<pre>GET /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx/share?method=put&accessorID=abc123&accessorObjCode=USER&coreAction=VIEW</pre>To retrieve the existing access rules:
...
<pre>POST /attask/api/&lt;supported-version&gt;/project?name=New Project</pre>The response includes the newly-created project along with its new ID and any other fields specified.
...
<pre>POST /attask/api/&lt;supported-version&gt;/upload</pre>The API expects the content type to be multipart/form-data. The parameter name for the file must be uploadedFile. The server returns the following JSON data:
...
<pre>POST /attask/api/&lt;supported-version&gt;/document?updates={<br>&nbsp;&nbsp;&nbsp;&nbsp;name: aFileName,<br>&nbsp;&nbsp;&nbsp;&nbsp;handle: abc...123, (handle from the file upload)<br>&nbsp;&nbsp;&nbsp;&nbsp;docObjCode: PROJ, (or TASK, OPTASK, etc)<br>&nbsp;&nbsp;&nbsp;&nbsp;objID: abc...123,<br>&nbsp;&nbsp;&nbsp;&nbsp;currentVersion:{version:v1.0,fileName:aFileName}<br>}</pre>
...
<pre>PUT /attask/api/&lt;supported-version&gt;/project/4c7...?name=New Project Name <br>PUT /attask/api/&lt;supported-version&gt;/project?id=4c7...&name=New Project Name</pre>Since update requires an ID, this operation will fail (without insertion) if the object does not exist on the server.
...
<pre>PUT /attask/api/&lt;supported-version&gt;/task/4c7.../move?projectID=5d8...</pre>An example for each action type is provided here: (??)
<pre>PUT /attask/api/&lt;supported-version&gt;/project/1234/approveApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateFinance<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateTimeline<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/calculateDataExtension<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/recallApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/project/1234/rejectApproval<br><br>PUT /attask/api/&lt;supported-version&gt;/task/1234/move<br><br>PUT /attask/api/&lt;supported-version&gt;/workitem/1234/markViewed</pre>Only the move action requires identifying additional attributes to specify the project where the work item is to be moved.
...
<pre>PUT /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx/share?accessorID=123abcxxxxxxxxxxxxxxxxxxxxxxxxxx&accessorObjCode=TEAMOB</pre>When editing an object, you can replace all access rules on an object by doing a PUT and sending updates similar to the following example:
<pre>PUT /attask/api/&lt;supported-version&gt;/project/123abcxxxxxxxxxxxxxxxxxxxxxxxxxx?method=PUT&updates={accessRules:[{accessorID:'123abcxxxxxxxxxxxxxxxxxxxxxxxxxx',accessorObjCode:'TEAMOB',coreAction:'VIEW'}]}</pre>The following example&nbsp;shows the syntax for moving a task from one project to another:
...
<pre>PUT /attask/api/&lt;supported-version&gt;/proj?Umethod=PUT&updates=[{"ID":"123abcxxxxxxxxxxxxxxxxxxxxxxxxxx","name":"Test_Project_1_ Edit"},{"ID":"123abcxxxxxxxxxxxxxxxxxxxxxxxxxx","name":"Test_Project_2_Edit"}]&apiKey=123abcxxxxxxxxxxxxxxxxxxxxxxxxxx</pre>which results in a return similar to the following:
