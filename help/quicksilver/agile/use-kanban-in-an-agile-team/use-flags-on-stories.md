...
On the [!DNL Kanban] board, flags provide a visual indication of when a story is ready to move to the next status. This enables Kanban teams to use a "pull" approach rather than a "push" approach when moving stories across statuses.
...
* Flags do not appear on any story cards that are in the [!UICONTROL Backlog] column or in the [!UICONTROL Complete] column (or in any column where the status of the column equates with Complete).
