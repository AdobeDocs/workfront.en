...
The default status shows which notifications (daily, [!UICONTROL instant], or both) are enabled by default for new users when you create the new users.
...
>[!NOTE]
>
>Workfront does not send any Daily digest notifications for the [!UICONTROL Miscellaneous] and [!UICONTROL Goals] categories of events. You cannot disable the Daily notifications for these categories.
...
The notifications in the [!UICONTROL Communication] category alert you to comments that have been posted in the Update stream of a specific item.
...
* Restore an item: When the Workfront administrator restores an object from the [!UICONTROL Recycle] Bin, an email is sent to the Workfront administrator.
