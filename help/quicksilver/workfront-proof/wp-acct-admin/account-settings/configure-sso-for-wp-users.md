...
The Single Sign-On functionality can be enabled on the [!UICONTROL Single sign-on] tab of your Account settings,&nbsp;and it will apply to all the users on your [!DNL Workfront] Proof account. See [Account Settings](https://support.workfront.com/hc/en-us/sections/115000912147-Account-settings) for more information.
...
**[!DNL https]://www.yourcompany.com/services/logout.asp**
...
1. Click **[!UICONTROL Edit],** populate the field and then click **Save** (2).

1. On the **SSO** row, click **[!UICONTROL Enabled]** (3).
...
1. If a user accesses [!DNL Workfront] Proof through the **default log in page** ([[!DNL https]://www.proofhq.com/login](https://www.proofhq.com/login)) there will be **two levels of authorization**. First a user is asked to log in using [!DNL Workfront] Proof access data (email and password). Then, the user is transferred through an SSO window (2) to the SSO login page.\
   Therefore, with SSO service enabled, we recommend to log in through your own [!DNL Workfront] Proof sub-domain/domain.
