...
# Document reprocessing for `proofing` {#document-reprocessing-for-proofing}

When you submit a document (DOCX, PDF, XLSX, AI) for `proofing`, `Workfront` reprocesses it so that it can display in the `proofing viewer` without the software application you used to create it.&nbsp;
