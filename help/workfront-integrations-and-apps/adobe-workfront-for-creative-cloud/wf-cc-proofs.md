...
<!--
Click the Menu icon in the top-right corner, then select Work List. You can also use the menu to navigate to parent objects. Go to the work item where you want to upload a proof. Click the Document icon in the navigation bar. Click New File near the bottom of the plugin. Enable the Create a proof toggle, then choose the Asset Type from the drop-down menu. In the Proof Approvals section, select Automated. (Optional) In the Workfrlow Template box, type the name of a proof workflow template. (Optional) Type a comment in the Updates area. (Optional) Type a name for the proof in the Proof Name text box. (Optional) Select Add outside file to add a file from your computer. Click Upload. The document appears in the Documents area in the plugin and the desktop app.
-->
