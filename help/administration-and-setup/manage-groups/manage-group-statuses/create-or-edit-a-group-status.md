...
<ol> 
 ...
 <li value="6"> <p>(Conditional) If the status is an issue status, make sure that <span class="bold">Master List</span> is selected.</p>  <p>For information about customizing the other issue types (Bug Report, Change Order, Issue, Request), see <a href="../../../administration-and-setup/set-up-workfront/configure-system-defaults/customize-default-issue-types.md" class="MCXref xref">Customize default issue types</a>.</p> </li> 
 <li value="7"> <p>(Conditional) To create a new status, click <span class="bold">Add a New Status</span>.</p> <p>Or</p> <p>To edit an existing status, mouse over the status you want to edit, then click the <span class="bold">Edit</span> option that displays to the far right.</p>  <note type="note"> 
   <ul> 
  ...
  <table cellspacing="0"> 
   ...
   <tbody> 
    ...
    <tr> 
     ...
     <td> <p>(Project and Task statuses only)</p> <p>Enable this option if you want the status hidden from users. When it is disabled (the default setting), all subgroups below the group can use the status.</p> <note type="tip">
