...
# Configure *`Workfront`* with Adobe Experience Manager {#configure-workfront-with-adobe-experience-manager}
...
## *`Workfront`* for AEM Assets {#workfront-for-aem-assets}
...
*  Collaborate and manage creative content by linking AEM assets and folders to projects, tasks, issues, and requests in *`Workfront`*. 
  ...
  For more information about configuring documentation integrations with third-party applications, see&nbsp; [Configure document integrations](configure-document-integrations.md).
...
*  Integrate with the AEM Digital Asset Management (DAM) repository, allowing you to use *`Workfront`* to manage and share digital assets stored in the DAM. 
  ...
  For more information about linking documents and asset folders, see &nbsp; [Link documents from external applications](link-documents-from-external-apps.md).
...
*  View an all-inclusive communication stream for an asset. Updates and comments made to an asset either in *`Workfront`* or AEM Assets are synchronized to the&nbsp;other application, establishing a comprehensive history of communications made to the&nbsp;asset.
  ...
  For more information about making comments in *`Workfront`*, see [Add an update to a document](add-update-documents.md).





## Prerequisites for installing the AEM Assets connector {#prerequisites-for-installing-the-aem-assets-connector}
...
1.  Click `Upload Package.`
   ...
   ` ![aem_package_manager_upload.png](assets/aem-package-manager-upload-350x93.png)
...
1.  Click `Install.`
   ...
   ` ![InstallConnector.png](assets/installconnector-350x119.png)  
   ` 
...
### Install the connector package on AEM assets version 6.4 (Touch UI) {#install-the-connector-package-on-aem-assets-version-touch-ui}
...
1. (Conditional) If you have not yet done so, create a *`Workfront`* cloud configuration file. 
    ...
    1. Click   `Create` in the upper-right corner of the Global- *`Workfront`* page.
       ...
       ` ![WFintegrationConfig2.png](assets/wfintegrationconfig2-350x173.png)    
    ...
    1. (Conditional) Click the `Advanced` tab in the upper-left corner of the *`Workfront`* Integration Configuration page, and select the following options if applicable:
       ...
       `Restrict Access:` Select this option to specify the appropriate *`Workfront`* IP addresses that need to be added to the allowlist. For more information about the allowlist, see [Configure your firewall](configure-your-firewall.md).
...
1.  (Optional) Synchronize bi-directional communication between AEM and *`Workfront`*. 
    ...
    1.  (Optional) To turn off comment synchronization, click `Disable Comment Sync.`
       ...
       For information on event subscriptions, see [Event Subscription API](event-subs-api.md).
...
### Install the connector package on AEM Assets versions 6.3 or older (classic UI)&nbsp; {#install-the-connector-package-on-aem-assets-versions-or-older-classic-ui}
...
1.  In the `Available Configurations` section, click `default (workfront-dam integration configuration)`.
   ...
   ![WFconfig.png](assets/wfconfig-350x150.png)
...
1.  (Conditional) Select the following options, if applicable for your configuration:  
   ...
   `Restrict Access:` Select this option to specify the appropriate *`Workfront`* IP addresses that need to be added to your allowlist. For more information about the allowlist, see the [Configure your firewall](configure-your-firewall.md) section in the article [Configure your firewall](configure-your-firewall.md).
...
1.  (Conditional) If no AEM API Key displays, click `Generate Key` to generate an API Key.
...
1.  (Optional) To turn off comment synchronization, click `Disable Comment Sync.`
   ...
   Delete the&nbsp;NOTE CREATE event subscription registered to your AEM instance.  
   For information on event subscriptions, see the [Event Subscription API](event-subs-api.md) section in the article [Event Subscription API](event-subs-api.md).
...
1.  Log in to *`Workfront`* as a *`Workfront administrator`*. 
   ...
   >[!TIP] {type="tip"}
   >
   >*`Workfront`* recommends creating a *`Workfront administrator`* dedicated solely to your AEM integration. For more information about assigning the *`Workfront administrator`* access level to a user, see [Grant users administrative access to certain areas](grant-users-admin-access-certain-areas.md).
...
For more information about *`Workfront`* permissions, see [Create or modify custom access levels](create-modify-access-levels.md).
...
### Set up users in AEM assets version 6.4 {#set-up-users-in-aem-assets-version}
...
### Set up users in AEM assets versions 6.3 or older&nbsp; {#set-up-users-in-aem-assets-versions-or-older}
...
1.   (Conditional) If the user is not a member of a *`Workfront`* group, in the `Add User to Groups` section, begin typing the name of a *`Workfront`* group to which the user belongs, then select the group when it displays in the drop-down menu.
   ...
   The user must belong to at least one *`Workfront`* group that has Access levels that include the Create and Delete permissions. For information about *`Workfront`* groups, see [Groups overview](groups.md). 
