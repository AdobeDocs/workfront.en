  ...
  <table cellspacing="0"> 
   ...
   <tbody> 
    <tr> 
     ...
     <td>Type a label for the field. This is what users will see when they use the custom form. The field <b>Name</b>, which fills in automatically, is referenced by <span>Workfront</span> in reports.</td> 
    </tr> 
    <tr> 
...
<ul> 
 <li> <p>Field names are case-sensitive and must appear in the calculation exactly how they appear in the <span>Workfront</span> system.</p> </li> 
 <li>You can use both built-in <span>Workfront</span> fields and custom fields that you already created.</li> 
 ...
 <li> <p>When you reference another custom field in a calculated custom field, the name of the field appears as it displays in the <span>Workfront</span> user interface. For example, when referencing a Typeahead-type custom field, you would use the following calculation to reference the name of the object selected in a field labeled "Developer name:" <code>UPPER({DE:Developer name:name})</code></p> <note type="note"> 
   <p>If you manually enter the name of a custom field by the same name as a <span>Workfront</span> native field, you must precede it in the calculation by "DE:" and surround it by curly brackets to indicate that you are referencing another custom field.</p> 
