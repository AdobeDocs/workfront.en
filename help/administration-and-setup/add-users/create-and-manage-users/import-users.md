...
## Synchronize users from a directory service {#synchronize-users-from-a-directory-service}

For information about how to synchronize users to your *`Workfront`* site via LDAP, see [Configure Adobe Workfront with LDAP](configure-workfront-ldap.md).


## Use a spreadsheet import file to import users {#use-a-spreadsheet-import-file-to-import-users}
...
1. Click the `New Person` drop-down arrow, then click `Import People`.

1.  In the `Import People` box that displays, download the sample file, then update the sample file to include your own user's personal information.
...
1. Click `Choose File` in the `Import People` box.

1. Navigate to and select the file that you saved.  
1.  (Optional) Select the `Send an invite email to this person` option to send an email invitation to the user, notifying them that a *`Workfront`* account has been created and prompting them to set their password.
