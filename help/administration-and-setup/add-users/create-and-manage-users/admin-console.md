...
<ol> 
 ...
 <li value="3"> <p>In the list that displays, with the <span class="bold">Product Profiles</span> tab selected, click the name of the <em>Workfront</em> Product Profile link. </p> <p> <img src="assets/prod-profile-350x127.png" style="width: 350;height: 127;"> </p> <p>This list includes all users that are already assigned to your Production instance of <em>Workfront</em>. </p> <note type="important">
...
<ol> 
 ...
 <li value="3"> <p>In the <b>Add users to this product profile</b> box, enter the email address or name of a user you want to add, then select <b>Save</b>. </p> <p>The user is created in <em>Workfront</em> with the Requestor access level. </p> <note type="important">
   Do not make any changes to the Product Profile itself.
  </note> </li> 
 <li value="4"> <p>In <em>Workfront</em>, change the user's access level.</p> <p>For instructions on how a <em>Workfront administrator</em> can change the user's access level, see <a href="../../../administration-and-setup/add-users/create-and-manage-users/edit-a-users-profile.md" class="MCXref xref">Edit a user's profile</a>.</p> </li> 
...
<ul> 
 <li> <p>Workfront System Administrators can deactivate a Workfront user from within <em>Workfront</em>, but this does not deactivate the user in the Admin Console.</p> <draft-comment>
   <p data-mc-conditions="QuicksilverOrClassic.Draft mode">For information about deactivating a user in <em>Workfront</em>, see </p>
  </draft-comment><p data-mc-conditions="QuicksilverOrClassic.Draft mode">For information about deactivating a user in <em>Workfront</em>, see </p> </li> 
 ...
 <li> <p>To remove System Administrator access from a user in <em>Workfront</em>, you need to use the Adobe Admin Console to remove the user as Product Profile Administrator. This changes the user's <em>Workfront</em> access level from System Administrator to Requestor.</p> </li> 
...
1. From the [Admin Console page](https://adminconsole.adobe.com/), select the **Products** tab and then select the **Workfront** product tile.
1. Select the link to the Workfront instance you want to change.
1. Select the Product profile link. This shows a list of the currently-assigned users. If the list is very long, you can also search for users in the search field above the list.
1. Select the **Add User** button.
1. In the **Add users** box, enter the email address or name of the user you want to add. Select **Save**. The administrator is created in Workfront with **Requestor** access level.

## Create system administrators

To create system administrators:

1. Make product profile assignments first. To be a Workfront System Administrator, the user must be assigned the Workfront product profile and be an admin for that product profile.
1. From the console, select the **Products** tab and then select the **Admins** tab. 
1. Select **Add Admin**.
1. In the **Add product profile administrators** box, enter the email address or name of the administrator you want to add. Select **Save**. The user is created in Workfront with **Requestor** access level.

## Additional details for the Admin Console:
...
* All new users are are assigned **Requestor** access level upon creation. Also, the user **Home Group** is determined based on the user who created them. This is currently not customizable from within the Admin Console.
...
* To remove Workfront system admin access, remove users as Product Profile Administrators. This action changes the user access level in Workfront from a system admin to a **Requestor**.
