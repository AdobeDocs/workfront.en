...
# Add stories to an existing iteration {#add-stories-to-an-existing-iteration}

You can add stories to an iteration in any of the following ways:



* From the backlog after the iteration is created, as described in the [Move stories from the backlog to an iteration or Kanban board](manage-the-agile-backlog.md#moving-stories-from-the-backlog) section in [Manage the agile backlog](manage-the-agile-backlog.md)
...
## Understand how adding stories affects task dates {#understand-how-adding-stories-affects-task-dates}
...
### `Task Planned Start Date`  {#task-planned-start-date}
...
### `Task Planned Completion Date`  {#task-planned-completion-date}
...
You can configure individual Scrum teams to use the project dates by default, rather than the iteration dates. For information, see the section [Configure how dates are applied when adding work items to an iteration](configure-scrum.md#configur5) in the article [Configure Scrum](configure-scrum.md). 


## Add a story to an existing iteration {#add-a-story-to-an-existing-iteration}
...
### `From the tasks or issues tab`  {#from-the-tasks-or-issues-tab}
