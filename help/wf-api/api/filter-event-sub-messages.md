...
# Filter event subscription messages {#filter-event-subscription-messages}
...
To learn about event subscriptions, see [Event Subscription API](event-subs-api.md).
...
1. Upon invocation of the handleRequest method, obtain the “newState” attribute of the event subscription message, which represents the updated state of the resource.  
   `<pre><span class="wysiwyg-font-size-small">public String handleRequest(Map<String, Object> webHookPayload, Context context) {        ...        Map<String, Object> newState = (Map<String, Object>) webHookPayload.get("newState");        ...}</span></pre>` To learn about the newState format, see [Outbound message format for event subscriptions](message-format-event-subs.md).
...
1. Obtain the “newState” attribute of the event subscription message.  
   ...
   `<pre><span class="wysiwyg-font-size-small">new_state = json.loads(event_subscription_message['newState'])</span></pre>` To learn about the newState format, see [Outbound message format for event subscriptions](message-format-event-subs.md).
...
1. Obtain the projectGroupID&nbsp;from the “newState” attribute of the event subscription message, then match it against the group ID of&nbsp;the group you identified in Step 1.  
   `<pre><span class="wysiwyg-font-size-small">let projectGroupId = eventSubscriptionMessage.newState.groupID;</span></pre>` To learn about the newState format, see [Outbound message format for event subscriptions](message-format-event-subs.md).
...
`To learn more about the retry mechanism, see [Event subscription retries](event-sub-retries.md).` 


###  

## Implementing Hosted Filters in Cloudless Architecture {#implementing-hosted-filters-in-cloudless-architecture}

{#implementing-hosted-filters-in-cloudless-architecture-1}
...
### Adjusting Filtering Examples for Stand-alone Services {#adjusting-filtering-examples-for-stand-alone-services}
...
### Preventing Missed Event Subscription Messages {#preventing-missed-event-subscription-messages}
