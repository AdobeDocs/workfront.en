...
# FAQs - Event Subscriptions {#faqs-event-subscriptions}
...
## Am I able to select which events are sent to an endpoint based on certain criteria within an event payload? {#am-i-able-to-select-which-events-are-sent-to-an-endpoint-based-on-certain-criteria-within-an-event-payload}

Event subscription filters are a way that event subs can be sorted by specified criteria. It is recommended that you apply filters to event subscriptions as it may significantly reduce the number of messages that an endpoint needs to consume. For more information see [Event subscription filtering](event-subs-api.md#event).


## Why is the API returning a 409 conflict response code? {#why-is-the-api-returning-a-conflict-response-code}
...
## What should I do if my messages aren't being delivered to my endpoint? {#what-should-i-do-if-my-messages-arent-being-delivered-to-my-endpoint}

Look for the following scenarios and use the recommended solution:



* Ensure that your subscription endpoint—defined by the `url` field—is returning a 2XX HTTP response code. If it is not, contact *`Workfront`* Support or see [Event Subscription delivery requirements](setup-event-sub-endpoint.md).
...
## Why is it taking an excessive amount of time for my messages to reach my endpoint? {#why-is-it-taking-an-excessive-amount-of-time-for-my-messages-to-reach-my-endpoint}
...
## What should I&nbsp;do if I'm receiving a 500 response status when I attempt to call the Event Subscription API? {#what-should-i-do-if-im-receiving-a-response-status-when-i-attempt-to-call-the-event-subscription-api}

Please contact *`Workfront`* Support. To learn how to contact support, see [Contact Customer Support](contact-customer-support.md).


## What different types of authentication can I use with *`Workfront`* Event Subscriptions? {#what-different-types-of-authentication-can-i-use-with-workfront-event-subscriptions}
...
## How long should it be before I receive my event payload from *`Workfront`* Event Subscriptions? {#how-long-should-it-be-before-i-receive-my-event-payload-from-workfront-event-subscriptions}
...
* `API Documentation`: [Event Subscription API](event-subs-api.md)

* `Best practices`: [Event subscription best practices](event-sub-best-practice.md)

* `Fields that trigger Event Subscription payloads`: [Event subscription resource fields](event-sub-resource-fields.md)

* `Understanding Event Subscription retries`: [Event subscription retries](event-sub-retries.md)

* `Configuring your firewall for *`Workfront`*`: [Configure your firewall](configure-your-firewall.md)
