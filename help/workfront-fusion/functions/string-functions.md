...
## ascii (text; [remove diacritics]) {#ascii-text-remove-diacritics}
...
## contains (text; search string) {#contains-text-search-string}
...
## escapeMarkdown(text) {#escapemarkdown-text}
...
## indexOf (string; value; [start]) {#indexof-string-value-start}
...
## length (text or buffer) {#length-text-or-buffer}
...
## replace (text;search string; replacement string) {#replace-text-search-string-replacement-string}
...
For more information on regular expressions, see [Text parser](text-parser.md).


## sha1 (text; [encoding]; [key]) {#sha-text-encoding-key}
...
## sha256 (text; [encoding]; [key]) {#sha-text-encoding-key-1}
...
## sha512 (text; [output encoding]; [key]; [key encoding]) {#sha-text-output-encoding-key-key-encoding}
...
## substring (text; start;end) {#substring-text-start-end}
