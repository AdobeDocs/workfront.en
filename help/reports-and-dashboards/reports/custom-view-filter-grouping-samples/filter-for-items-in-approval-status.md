...
# Filter: display only items in an approval status {#filter-display-only-items-in-an-approval-status}
...
## Display only items in approval status {#display-only-items-in-approval-status}
